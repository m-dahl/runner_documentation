...
\node [proc, join=by free] {Find plan to state where spec holds};
\node [proc, join=by free] (p5) {Suggest actions to user};
...
\draw [*->,lcfree] (t2.east) -- (c2) |- (t6);

\path (t3.east) to node [yshift=-1em] {$n$} (c3);
\draw [o->,lcfree] (t3.east) -- (c3) |- (t6);
